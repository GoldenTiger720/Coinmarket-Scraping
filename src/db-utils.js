...
export async function weekInsert(db, table, items, chunkSize = 999) {
  for (let i = 0; i < items.length; i += chunkSize) {
    await db
      .insert(table)
      .values(items.slice(i, i + chunkSize))
...
export async function coinInsert(table, items, chunkSize = 999) {
  for (let i = 0; i < items.length; i += chunkSize) {
    await db
...
export async function top30assetsInsert(table, items, chunkSize = 999) {
  for (let i = 0; i < items.length; i += chunkSize) {
    await db.insert(table).values(items).onConflictDoNothing();
  }
}

export async function portfolioInsert(db, table, items) {
  await db.insert(table).values(items).onConflictDoNothing();
}

export async function getTop30assets(db, table, weekStart) {
  const result = await db.select().from(table).run();

  return result;
}

export async function getPortfolioValue(db, table) {
  const result = await db.select().from(table).run();
  return result;
}
...
export const getSpotBySymbol = handle(async (symbol) => {
  const result = await db
    .select()
    .from(spot)
    .where(eq(spot.symbol, symbol))
    .orderBy(spot.timestamp)
    .execute();
  return result;
}, "getting all spot prices");
